...
## Check non-English characters (for Carrier feature)
def isEnglish(s):
	try:
		s.encode('ascii')
		return True
...
def checkFile(datapath, original_filename, max_num_att ):
	...
	if rewrite:
		...
		#print "Rewrite a file :"+datapath+new_filename
		for line in lines:
			arr = line.strip().split(",") 
 			if len(arr) > max_num_att:
				## ignote att#19 
				...
				## skip attribute #19
				for i in range(20, max_num_att+1):
					row.append(arr[i]) 
		...
		## If a file is valid (no extra features), will copy it to a new file. (Or just rename it) 
...
def threedotnumber(dotted_number, need2num=False):
	...
	## Append zero at the end if a number has 2 dots. For App version feature
...
def transformFeature(datapath, checked_fname):
 	
 	## a file contains a mapping from IP to country
	gi = pygeoip.GeoIP(datapath+"/GeoLiteCity.dat")
	...
	numerical_arr_list = ["NumCampaignMatch","ScreenHeight","NumCrash","DailyUsage","ScreenWidth","InstallDays",
	"PushCount","Questions","CorrectQuestion"]
	
	##  A list of categorical features
	string_arr_list = ["Carrier","DeviceModel","OS","UserType"]
	
	##  A list of datetime features
 	datetime_arr_list = [ "StartDate", "LastUpdateDate","BlockPush","RevokePush","Uninstalled"]
	
	##  A list of categorical features that have special format and need to be transformed
	special_format = ["AppVersion", "OSVersion","Timezone"] 
	
	## Excluded features when  clustering
	...
	for line in f_r.readlines():
		...
		norm_arr.append( arr[0])
 		...
 		while i<len(arr):
			...
			elif att_name[i] in  numerical_arr_list:
